...
test("should sort rolls", t => {
  ...
  let values = parseAll(result).lines.map(line => line.value);
  t.true(
    values[0] >= values[1] &&
      values[1] >= values[2] &&
      values[2] >= values[3] &&
...
test("should not include bots", t => {
  ...
  let result = parseAll(Roll.group(members.concat(bot)));
  let botResults = result.lines.filter(line => line.name === bot.name);
...
test("should not include offline or afk", t => {
  ...
  let result = parseAll(Roll.group(members.concat([offline, idle])));
  let filteredResults = result.lines.filter(
    line => line.name === "offline" || line.name === "idle"
  );
...
test("should respect die size args for group", t => {
  ...
  let result = parseAll(Roll.group(members, 1));
  let values = result.lines.map(line => line.value);
...
test("should respect die size args for individual roll", t => {
  ...
  for (let i = 0; i < 100; i++) {
    let result = parseOne(Roll.one(member, 1));
